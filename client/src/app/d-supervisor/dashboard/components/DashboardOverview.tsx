...
export function DashboardOverview({
  ...
  const metrics = [
    {
      ...
      value: totalStudents,
      hint: "Students currently under supervision",
      trend: "up" as const,
    },
    {
      label: "Pending Reviews",
      value: totalPendingReviews,
      hint: "Reviewed logbooks waiting for final approval",
  ...
  const levelCountMap = students.reduce((acc: Record<string, number>, student: any) => {
    const levelLabel = student.level ? `${student.level} Level` : "Unknown";
    acc[levelLabel] = (acc[levelLabel] || 0) + 1;
    return acc;
  }, {});

  const levelData = Object.entries(levelCountMap).map(([level, count]) => ({
    level,
    students: count,
  }));
  ...
  return (
    <div className="space-y-4 md:space-y-5">
      ...
      <DashboardWelcomeBanner
        title="Academic Supervisor Workspace"
        description="Track student supervision, finalize reviewed logbooks, and monitor placement progress from one place."
        action={
          <Button asChild className="h-9">
            <Link href="/d-supervisor/logbooks">Open Logbooks</Link>
          </Button>
        }
      />
      ...
      <section className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <div className="space-y-4 lg:col-span-2">
          ...
          >
            {levelData.length > 0 ? (
              <DashboardTrendLineChart data={levelData} xKey="level" yKey="students" />
            ) : (
              <div className="h-full flex items-center justify-center">
                <EmptyState
                  title="No student level data"
              ...
              <div className="space-y-3">
                ...
                {students.length > 5 ? (
                  <Button asChild variant="outline" className="h-9">
                    <Link href="/d-supervisor/students">View All Students</Link>
                  </Button>
                ) : null}
              </div>
            ) : (
              ...
              {totalPendingReviews > 0 ? (
                <div className="space-y-3 rounded-md border border-border bg-muted/20 p-3">
                  <p className="text-sm text-foreground">
                    ...
                    {totalPendingReviews > 1 ? "s" : ""} awaiting your approval.
                  </p>
